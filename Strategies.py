...
def rsi_strategy(symbol):
    # Calculamos el RSI
    rsi = ind.calculate_rsi(symbol, mt5.TIMEFRAME_H1, 14)
...
def ma_rsi_strategy(symbol, timeframe, ma_fast_period, ma_slow_period, rsi_period, rsi_buy_threshold, rsi_sell_threshold):
    # Obtener precios de cierre
    close_prices = ind.get_close_prices(symbol, timeframe, ma_slow_period)

    # Calcular MA rápida y lenta
    ma_fast = ind.calculate_ma(close_prices, ma_fast_period, talib.MA_Type.SMA)
    ma_slow = ind.calculate_ma(close_prices, ma_slow_period, talib.MA_Type.SMA)
...
def hichimoku_strategy(symbol, type, price, lot, deviation, tenkan_sen, kijun_sen, senkou_span_a, senkou_span_b, chikou_span):
    ...
    if price > kijun_sen[-1]:
        ...
        if price > senkou_span_a[-1] and price > senkou_span_b[-1]:
            ...
            if tenkan_sen[-1] > kijun_sen[-1]:
                ...
                Main.order(symbol, mt5.ORDER_TYPE_BUY, price, lot, deviation, sl, tp)
    ...
    # Si el precio está por debajo del kijun_sen, comprobamos si está por debajo de la nube y si el tenkan_sen está por debajo del kijun_sen
        ...
        if price < senkou_span_a[-1] and price < senkou_span_b[-1]:
            if tenkan_sen[-1] < kijun_sen[-1]:
                ...
                Main.order(symbol, mt5.ORDER_TYPE_SELL, price, lot, deviation, sl, tp)
...
def ma_rsi_macd_strategy(symbol, timeframe, ma_fast_period, ma_slow_period, rsi_period, rsi_buy_threshold, rsi_sell_threshold, macd_fast_period, macd_slow_period, macd_signal_period):
    # Obtener precios de cierre
    close_prices = get_close_prices(symbol, timeframe, ma_slow_period)

    # Calcular MA rápida y lenta
    ma_fast = calculate_ma(close_prices, ma_fast_period, talib.MA_Type.SMA)
    ma_slow = calculate_ma(close_prices, ma_slow_period, talib.MA_Type.SMA)

    # Calcular RSI
    rsi = calculate_rsi(close_prices, rsi_period)

    # Calcular MACD
    macd, signal = calculate_macd(close_prices, macd_fast_period, macd_slow_period, macd_signal_period)
...
def ichimoku_stochastic_strategy(symbol, timeframe, k_period, d_period, tenkan_period, kijun_period, senkou_period):
    ...
    close_prices = get_close_prices(symbol, timeframe, senkou_period)
    ...
    tenkan_sen, kijun_sen, senkou_span_a, senkou_span_b, chikou_span = calculate_hichimoku(high_prices, low_prices, close_prices)

    # Calcular Stochastic
    slowk, slowd = calculate_stochastic(high_prices, low_prices, close_prices, k_period, d_period)
...
def ma_rsi_stoch_strategy(symbol, timeframe, ma_fast_period, ma_slow_period, rsi_period, rsi_buy_threshold, rsi_sell_threshold, stoch_k_period, stoch_d_period, stoch_slowing_period):
    ...
    # Calcular Stochastic
    stoch_k, stoch_d = calculate_stochastic(close_prices, stoch_k_period, stoch_d_period, stoch_slowing_period)
...
def ma_macd_strategy(symbol, timeframe, ma_fast_period, ma_slow_period, macd_fast_period, macd_slow_period, macd_signal_period):
    ...
    # Calcular MACD
    macd, macd_signal, _ = calculate_macd(close_prices, macd_fast_period, macd_slow_period, macd_signal_period)
